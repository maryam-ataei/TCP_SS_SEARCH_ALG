/* Change:
SEARCH2.0 RFC type
* Make it like the pseudocode in SEARCH RFC

Only use delivered bytes
For bin calculation: use delta
...
#define SEARCH_VERSION 20 /*Version 2.0 like RFC*/
...
struct bictcp {
	...
	s32	curr_idx; 			/* total number of bins */ //NEW
...
static void search_update_bins(struct sock *sk, u32 now_us)
{
	...
	if (passed_bins >= SEARCH_TOTAL_BINS)
		...
		for (i = ca->curr_idx + 1; i <= ca->curr_idx + passed_bins; i++) //DIFFERENT (i = ca->curr_idx + 1)
	...
	ca->bin[ca->curr_idx % SEARCH_TOTAL_BINS] = tp->bytes_acked - ca->prev_bytes_acked;
	ca->prev_bytes_acked = tp->bytes_acked;
}
...
static inline u64 search_sum_bins(struct sock *sk, s32 index1, s32 index2, u32 fraction)
{
	...
	for (i = index1 + 1; i <= index2 - 1; i++)
		sum += bin[i % SEARCH_TOTAL_BINS];

	sum += bin[index1 % SEARCH_TOTAL_BINS] * fraction / 100;
	sum += bin[index2 % SEARCH_TOTAL_BINS] * (100 - fraction) / 100;
...
static void search_update(struct sock *sk, u32 rtt_us)
{
	...
	if (now_us > ca->bin_end_us) {	
		...
		if (prev_idx >= SEARCH_BINS && (ca->curr_idx - prev_idx) <= SEARCH_EXTRA_BINS) { 
			...
			fraction = (rtt_us % ca->bin_duration_us) * 100 / ca->bin_duration_us; //CHANGE
