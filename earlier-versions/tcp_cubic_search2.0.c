...
#define SEARCH_VERSION 20 /*Only for log*/
...
struct bictcp {
	...
	u16	not_enough_bin;	/* number of cases that  there is not enough bins in shift */ /*only for logging*/
	u16	missed_bin;	/*only for logging*/
	////////////////////////////////////////////////////////
};
...
// static void search_update_missed_bins(struct sock *sk, u32 now_us)
static void search_update_missed_bins(struct sock *sk, u32 now_us) // as I want to return missed bins for logging
{
	struct bictcp *ca = inet_csk_ca(sk);
...
static void search_exit_slow_start(struct sock *sk, u32 rtt_us, u32 now_us)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct bictcp *ca = inet_csk_ca(sk);
...
static u64 search_interpolate_delivered_bytes(struct sock *sk, u32 rtt_us, u32 curr_index, u32 prev_index, u64 left_bytes, u64 right_bytes, u32 now_us) {
...
static void search_update(struct sock *sk, u32 rtt_us)
{
	...
	if (now_us > ca->bin_end_us) {	
		...
		search_update_missed_bins(sk, now_us);
		...
		if (prev_index >= SEARCH_BINS && SEARCH_TOTAL_BINS - (curr_index - prev_index) >= SEARCH_BINS) { 
			...
			if (do_intpld == 1) 
				prev_delv_bytes = search_interpolate_delivered_bytes(sk, rtt_us, curr_index, prev_index, prev_delv_bytes_under, prev_delv_bytes_over, now_us);
			else
			...
			if (prev_delv_bytes > 0) {
				...
				if ((2 * prev_delv_bytes) >= curr_delv_bytes && norm_diff >= search_thresh) 
					search_exit_slow_start(sk, rtt_us, now_us);
			}
		}
		else if (prev_index > SEARCH_BINS)
			ca->not_enough_bin += 1;
