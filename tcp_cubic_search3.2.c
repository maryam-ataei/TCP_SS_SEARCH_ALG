/* Improvement:
SEARCH3.2
* Change window size, if we do not have enough bins

SEARCH V3.1:
Only use delivered bytes
For bin calculation: use cumulative bytes
Have set cwnd and interpolation as option
change window size, if not have enough bins
*/
...
#define SEARCH_TOTAL_BINS  (SEARCH_BINS + SEARCH_EXTRA_BINS) /* Total number of bins containing essential bins to cover RTT shift */	
...
static void search_update_bins(struct sock *sk, u32 now_us)
{
	...
	passed_bins = ((now_us - ca->bin_end_us) / ca->bin_duration_us) + 1;

	if (passed_bins >= SEARCH_TOTAL_BINS)
		    memset(ca->bin, ca->bin[ca->curr_idx % SEARCH_TOTAL_BINS], sizeof(ca->bin));

	else {
		for (i = ca->curr_idx + 1; i <= ca->curr_idx + passed_bins; i++){
			if (ca->curr_idx == -1)
				ca->bin[i % SEARCH_TOTAL_BINS] = 0;
			else
				ca->bin[i % SEARCH_TOTAL_BINS] = ca->bin[ca->curr_idx % SEARCH_TOTAL_BINS];
		}
	}
	
	ca->bin_end_us += passed_bins * ca->bin_duration_us;
	ca->curr_idx += passed_bins;

	ca->bin[ca->curr_idx % SEARCH_TOTAL_BINS] = tp->bytes_acked;
}

// Function to calculate delivered bytes for a window considering interpolation
static inline u64 search_sum_bins(struct sock *sk, s32 index1, s32 index2, u32 fraction)
{
	struct bictcp *ca = inet_csk_ca(sk);

	u64 sum = 0;

	sum = ca->search.bin[(index2 - 1) % SEARCH_TOTAL_BINS] - ca->search.bin[index1 % SEARCH_TOTAL_BINS];

	if (index1 > 0) // it happens only for the first time that the current index is less than 25.(As the index -1%25 is 24 and bin 24 does not exist)
		sum += (ca->search.bin[index1 % SEARCH_TOTAL_BINS] - ca->search.bin[(index1 - 1) % SEARCH_TOTAL_BINS]) * fraction / 100;
	else
		sum += (ca->search.bin[index1 % SEARCH_TOTAL_BINS]) * fraction / 100;

	sum += (ca->search.bin[index2 % SEARCH_TOTAL_BINS] - ca->search.bin[(index2 - 1) % SEARCH_TOTAL_BINS]) * (100 - fraction) / 100;
...
static void search_exit_slow_start(struct sock *sk, u32 now_us, u32 rtt_us)
{
	...
	if (cwnd_rollback == 1) {
		...
		cong_idx = ca->curr_idx - ((2 * initial_rtt) / ca->bin_duration_us);

		if (ca->curr_idx - cong_idx > SEARCH_TOTAL_BINS)
			cong_idx = ca->curr_idx - SEARCH_TOTAL_BINS + 1;

		overshoot = search_sum_bins(sk, cong_idx, ca->curr_idx, 0);

		rollback_cwnd = overshoot / tp->mss_cache;

	if (rollback_cwnd < tp->snd_cwnd)
		tp->snd_cwnd = max(tp->snd_cwnd - rollback_cwnd, TCP_INIT_CWND);
	}
	ca->stop_search = 1;
 	tp->snd_ssthresh = tp->snd_cwnd;
...
static void search_update(struct sock *sk, u32 rtt_us)
{
	...
	if (now_us > ca->bin_end_us) {	
		...
		prev_idx = ca->curr_idx - (rtt_us/ca->bin_duration_us);

		if (prev_idx >= SEARCH_BINS && (ca->curr_idx - prev_idx) <= SEARCH_EXTRA_BINS) { 
			
			/* Calculate delivered bytes for the current and previous windows */
			curr_delv_bytes = search_sum_bins(sk, ca->curr_idx - SEARCH_BINS, ca->curr_idx, 0);
			fraction = ((rtt_us % ca->bin_duration_us) / ca->bin_duration_us) * 100;
			prev_delv_bytes = search_sum_bins(sk, prev_idx - SEARCH_BINS, prev_idx, fraction);
		...
		else if (prev_idx > SEARCH_BINS) {

			overlap_bins = ca->curr_idx - prev_idx - SEARCH_EXTRA_BINS;

			// u16 new_search_window_size_time = (SEARCH_BINS - overlap_bins) * 10 / overlap_bins;

			// if new_search_window_size_time >= 2 {
			if (overlap_bins >= SEARCH_BINS / 2) {
				new_search_bins = SEARCH_BINS - overlap_bins;

				/* Calculate delivered bytes for the current and previous windows */
				curr_delv_bytes = search_sum_bins(sk, ca->curr_idx - new_search_bins, ca->curr_idx, 0);
				fraction = ((rtt_us % ca->bin_duration_us) / ca->bin_duration_us) * 100;
				prev_delv_bytes = search_sum_bins(sk, prev_idx - new_search_bins, prev_idx, fraction);
