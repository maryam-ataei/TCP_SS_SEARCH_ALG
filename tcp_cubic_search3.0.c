...
MODULE_PARM_DESC(search, "Enable the Slow start Exit At Right CHoke point (SEARCH) algorithm _ 0: disabled, 1: enabled");
...
static void search_update_bins(struct sock *sk, u32 now_us)
{
	...
	if (passed_bins >= SEARCH_TOTAL_BINS)
		    memset(ca->bin, ca->bin[ca->curr_idx], sizeof(ca->bin));

	else {
		for (i = ca->curr_idx + 1; i <= ca->curr_idx + passed_bins; i++){

			if (ca->curr_idx >= 0)
				ca->bin[i % SEARCH_TOTAL_BINS] = ca->bin[ca->curr_idx];
		}
	}
